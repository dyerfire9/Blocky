...
class Block:
    ...
    def smash(self) -> bool:
        ...
        if not self.smashable():
            ...
            new_size = self._child_size()
            max_depth = self.max_depth
            level = self.level + 1
            upper_left = Block((0, 0), new_size, None, level, max_depth)
            upper_right = Block((0, 0), new_size, None, level, max_depth)
            lower_left = Block((0, 0), new_size, None, level, max_depth)
            lower_right = Block((0, 0), new_size, None, level, max_depth)
            self.children = [upper_right, upper_left, lower_left,
                             lower_right]
            ...
            for child in self.children:
                ...
                if randomint < math.exp(-0.25 * self.level) and \
                        not child.smash():
                    child._random_colour()
                else:
                    child._random_colour()
            return True

    def _random_colour(self) -> None:
        """find a random colour for this block, and is a helper for .smash()"""
        randomcolour = random.randint(0, 3)
        if randomcolour == 0:
            self.colour = COLOUR_LIST[0]
        elif randomcolour == 1:
            self.colour = COLOUR_LIST[1]
        elif randomcolour == 2:
            self.colour = COLOUR_LIST[2]
        else:
            self.colour = COLOUR_LIST[3]
